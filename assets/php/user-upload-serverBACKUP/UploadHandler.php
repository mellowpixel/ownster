...
class UploadHandler
{
    ...
    public function post($print_response = true) {
        ...
        /*if(!isset( $_SESSION["created_products"] )){
            ...
            $_SESSION["created_products"] = array();
        }*/

        $product_id = $_SESSION["unique_user_id"];
        $product_data = array( $files );
        $found = false;
        ...
        foreach($jstr_arr as $json_str){
            ...
            if(isset($side[ $_files["files"]["name"][0] ])){
                $files[0]->side_name = $side[$_files["files"]["name"][0]];
            }
        }
        //
        ...
        if( isset($_SESSION["created_products"]) && array_key_exists($product_id, $_SESSION["created_products"]) ){
            if(in_array($_files["files"]["name"][0], $complete_product_img_names)){
                if($_files["files"]["name"][0] == $_post["default_surface"]){
                    $_SESSION["created_products"][$product_id]["data"]["default_pic"] = $files[0]->url;    
                }
                array_push( $_SESSION["created_products"][$product_id]["data"], $files[0]);
            }    

        } else {
            $_SESSION["created_products"] = array_combine(array($product_id), array(array("data"=>array("default_pic"=>""), "quantity"=>1, "product_db_id"=>null)));
            if(in_array($_files["files"]["name"][0], $complete_product_img_names)){
                if($_files["files"]["name"][0] == $_post["default_surface"]){
                    $_SESSION["created_products"][$product_id]["data"]["default_pic"] = $files[0]->url;
                }
        ...
        if(isset($_post["product_id"])){
            $_SESSION["created_products"][$product_id]["product_db_id"] = $_post["product_id"];
        }
