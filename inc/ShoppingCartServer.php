...
if( isset( $_POST ) ){
	...
	switch( key($_POST) ){ // Get Current key ( which is first )
		...
		case "search_vaucher":		searchVaucher($_POST["search_vaucher"], $db);	break;
...
function getDiscountSchemes(){
	...
	if($res){
		...
		recordMemoryUsage("ShoppingCart.php -> getDiscountSchemes");
		return json_encode(array("error"=>true, "error_msg"=> mysql_error()));
	}
...
if(isset($_POST["Submit_payment"])){
	...
	//	$total_copies	= 0;
		...
		//--------------------------------------------------------------------------//
		$custom_data = array("delivery_date"=>$delivery_date, "product_ids"=>$product_ids);
		if(isset($_SESSION["redeemed_voucher"])){
			$custom_data["voucher"] = $_SESSION["redeemed_voucher"];
			// file_put_contents("../Logs/php_output.log", print_r($_SESSION["redeemed_voucher"],true));
		}

		$custom_variable = json_encode($custom_data);
...
function submitOrderToEmail($post, $session, $db){
	/**/file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n\n ------------ ".date('H:i', time())." ".$post["email_order"]["address"]["email"].
							" Function Begins 'submitOrderToEmail' -----------\nSubmited Data:\n\$_POST = ".print_r($post, true), FILE_APPEND);
	...
	if(!$address_arr) {
		$address_arr = cleanAddress($post["email_order"]["address"]);
	}
	
	$items = $session->getSessionValue("orders_in_cart");
	// Merge new orders with reorders from history
	if(isset($_SESSION["orders_in_cart"]) && isset($_SESSION["orders_from_history"])){
        $items = array_merge($_SESSION["orders_in_cart"], $_SESSION["orders_from_history"]);
    } else if(isset($_SESSION["orders_from_history"]) && !isset($_SESSION["orders_in_cart"])) {
        $items = $session->getSessionValue("orders_from_history");
    ...
    // Convert order data into the object, create order info html body
    if($items){
    	// /**/file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n*  If Orders in cart exist. if(\$items) *\n\$items = ".json_encode($items), FILE_APPEND);
    	$order_id_qty = array();
		$orders_array = processOrder($items, $body, $order_id_qty);
		$orders = json_encode($order_id_qty); // $order_id_qty is edited inside processOrder function by reference
		array_push($fields, "items");
		array_push($values, "'$orders'");

		/*file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n*  Processed orders. \$orders_array = ".
								json_encode($orders_array).
								"\n\$orders = json_encode(\$orders_array)".
								"\n\$orders = ".$orders, FILE_APPEND);*/
	}
	...
	/* ATTENTION if(is_array($address_arr)) */
	if(is_array($address_arr)){
		...
		$street = (isset($address_arr["address2"])) ? "'".$address_arr["address1"]." ".$address_arr["address2"]."'" : "'".$address_arr["address1"]."'";

		array_push($fields, "first_name");		array_push($values, "'".$address_arr["name"]."'");
		array_push($fields, "last_name");		array_push($values, "'".$address_arr["lname"]."'");
		array_push($fields, "address_street");	array_push($values, $street);
		array_push($fields, "address_city");	array_push($values, "'".$address_arr["city"]."'");
		array_push($fields, "address_zip");		array_push($values, "'".$address_arr["postcode"]."'");
		array_push($fields, "address_country");	array_push($values, "'".$address_arr["country"]."'");
		array_push($fields, "payer_email");		array_push($values, "'$payer_email'");
		
		
	} else {
		/**/file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n*  Address Error if(is_array(\$address_arr)) *\n".print_r($address_arr, true), FILE_APPEND);
	}
	
	if($vouch_code){
		array_push($fields, "voucher_code");
		array_push($values, "'$vouch_code'");
		$vr = mysql_query("SELECT v_selling_price FROM vouchers WHERE voucher_code = '$vouch_code'");
		if($vr){
			$vouch_price = @mysql_result($vr, 0);
			array_push($fields, "mc_gross");
			array_push($values, $vouch_price);
		}
	}

	$datetime	= date("Y-m-d H:i:s"); //YYYY-MM-DD HH:MM:SS
	array_push($fields, "payment_date");
	...
	// /**/file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n*  Save Order into 'completed_payments' table. *\n\nFields:\n$fields_str\n\nValues:\n$values_str", FILE_APPEND);
	if(!mysql_query("INSERT INTO completed_payments ($fields_str) VALUES($values_str)")){
		// /**/file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n*  ERROR Saving Order into 'completed_payments' table. *\n\nFields:\n$fields_str\n\nValues:\n$values_str\n\nOrders Array:\n".json_encode($orders_array), FILE_APPEND);
		sendMail("mellowpixels@gmail.com", "OWNSTER ERROR ShoppingCartServer.php", "".mysql_error()." --- Fields = ".$fields_str."\nvalues = ".$values_str, "Dima", "dima@mellowpixels.com");
	}
	
	/* Bugs in $addr and $orders_array
	*/
	...
	// mail("mellowpixels@gmail.com"/**/, "Ownster.co.uk NEW ORDER", $body, $headers);
...
/*
--------------------------------------------------------------------------------------------------------------*/
function processOrder($items, &$body, &$order_id_qty){
	$orders_array = array();		
	$i=0;

    foreach ($items as $product) {
        foreach ($product as $id=>$data) { 
	  		$product_db_id = $data["product_db_id"]; // Database ID of the product
            ...
            $default_pic = movePicture($id, $data, $product_pics);

			$body.= "Item ".($i+1).":&nbsp;&nbsp;&nbsp;".$data["quantity"]." x Oyster Card Wallet <b style='color:#08A708'>".$id."</b><br/>";
			...
			$one_order = (object)array(
							...
							"order_date"=>date("d/m/Y"),
							"item_price"=>$price_name["price"],
							"order_total_price"=>$data["quantity"] * $price_name["price"],
							"product_name"=>$price_name["name"],
							"product_thubnails"=>array("default_pic"=>$default_pic,
														 "all_pics"=>$product_pics));

			array_push($orders_array, $one_order);
			array_push($order_id_qty, (object)array( "item_id"=>$id, "qty"=>$data["quantity"]));
		}
		$i++;
	}
...
/*
--------------------------------------------------------------------------------------------------------------*/
function movePicture(&$item_folder_name, &$data, &$product_pics){
	if(is_dir("../user_upload/". $item_folder_name ."/")){
       	move_file("../user_upload/". $item_folder_name ."/", "../paid_orders/". $item_folder_name ."/");
        // Save pathnames of thumbnails
        $default_pic = returnMovedDirPath($data["data"]["default_pic"]["url"], "/user_upload/$item_folder_name/", "/paid_orders/$item_folder_name/" );    
        foreach($data["data"] as $side){
        	if(is_object($side)){
        		// Save new path in orders history
			    $new_path = returnMovedDirPath($side->url, "/user_upload/$item_folder_name/", "/paid_orders/$item_folder_name/" );
				array_push($product_pics, array( "side_name"=>$side->side_name,
												 "side_picture"=>$new_path));
			}
		}
...
function sendOrderConfirmation($first_name, $last_name, $payer_email, $address, $delivery_date, $order_number, $vouch_code, $orders_array, $db){
	...
	if(!is_array($orders_array)){
		/**/file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n*** ERROR ***\n Order Confirmation error. Function: 'sendOrderConfirmation();'\n \$orders_array Must be array, but it is not.\n\$orders_array = ".print_r($orders_array,true), FILE_APPEND);
	}
...
function voucherGen($post_val){
	...
	for($i = 0; $i < $vaucher_qty; $i++ ){
		...
		if($exp_date){
			...
			$query = mysql_query( "INSERT INTO vouchers (voucher_code, ".$discount_db_field.", v_selling_price, expires_date, num_of_discounts, date_created) VALUES('".$vouch_code."', ".$discount_value.", $voucher_selling_price, '".$exp_date."', 0, '$date')" );
		} else {
			$query = mysql_query( "INSERT INTO vouchers (voucher_code, ".$discount_db_field.", v_selling_price, num_of_discounts, date_created) VALUES('".$vouch_code."', ".$discount_value.", $voucher_selling_price, ".$num_of_discount.", '$date')" );	
		}
	...
	if($result){
		if(mysql_num_rows($result) > 0){
			while($row = mysql_fetch_assoc($result)){
				...
				$date_created = ($row["date_created"] != null) ? date('d.m.y @ H:i',strtotime($row["date_created"])) : "-----";
...
function searchVaucher($voucher_code, $db) {
	preg_match_all('/[a-zA-Z0-9]/', $voucher_code, $matches);
	$clean_voucher = strtolower(implode($matches[0]));
	// file_put_contents("../Logs/php_output.log", implode($matches[0]));
	$result = mysql_query("SELECT * FROM vouchers WHERE voucher_code = '$clean_voucher'");
	if($result){
		if(mysql_num_rows($result)){
			$outp = mysql_fetch_assoc($result);
			$outp["date_created"] = ($outp["date_created"] != null) ? date('d.m.y @ H:i',strtotime($outp["date_created"])) : "-----";
			$outp["voucher_code"] = $vouch_code_defis = substr_replace($outp["voucher_code"], "-", 4, 0);
			echo json_encode(array("error"=>false, "output"=> $outp));
		} else echo json_encode(array("error"=>true, "error_msg"=> "Ваучер не найден."));
...
function validateAddress($submited_data){
	...
	if(!$error){
		...
		sendMail("mellowpixels@gmail.com", "OWNSTER ERROR ShoppingCartServer.php", "Address Validation error.\nSubmited Data:\n".print_r($submited_data, true), "Dima", "dima@mellowpixels.com");
		/**/file_put_contents("../Logs/ShoppingCartServer".date("d-m-y").".log", "\n\n*** ERROR ***\nAddress validation error. Function: 'validateAddress()'\nSubmited Data:\n".print_r($submited_data, true)."\n", FILE_APPEND);
		return false;
	}
}

function cleanAddress($submited_data){
	$address_array	= array();

	$address_array["name"] = stringCleanse($submited_data['name']);
	$address_array["lname"] = stringCleanse($submited_data['lastname']);
	$address_array["fullname"] = stringCleanse($submited_data['name'] ." ". $submited_data['lastname']);
	$address_array["address1"] = stringCleanse($submited_data['address']);
	
	if(isset($submited_data['address2']) && strlen($submited_data['address2']) > 0){
		$address_array["address2"] = stringCleanse($submited_data['address2']);
	}

	$address_array["city"] = stringCleanse($submited_data['city']);
	$address_array["postcode"] = stringCleanse($submited_data['postcode']);
	$address_array["country"] = stringCleanse($submited_data['country']);
